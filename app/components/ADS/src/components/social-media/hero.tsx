...
const inputClassName = "h-12 w-full rounded-xl border border-border bg-background px-4 text-sm text-foreground outline-none transition-all placeholder:text-muted-foreground focus:border-accent focus:ring-4 focus:ring-accent/15 disabled:cursor-not-allowed disabled:opacity-60";
...
export function Hero() {
  ...
  const onSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    const validationError = validateForm({ name, email, phone, brand, profile, service, requirements });
    ...
    try {
      const response = await fetch("/api/ads-enquiry", {
        ...
        body: JSON.stringify({ name, email, phone, brand, website, instagram, challenge: service, goals: requirements, source: "social-media-hero" }),
      });
        ...
        <div id="audit" className="lg:sticky lg:top-24">
          <div className="rounded-3xl border bg-card p-6 shadow-lift sm:p-8">
            ...
            <form onSubmit={onSubmit} className="mt-7 space-y-4" noValidate aria-busy={loading}>
              ...
              <Field label="Company or business name"><input required name="brand" placeholder="Your company or brand" className={inputClassName} autoComplete="organization" disabled={loading} /></Field>
              <Field label="Instagram handle or website"><input required name="profile" placeholder="@yourbrand or yourbrand.com" className={inputClassName} disabled={loading} /></Field>
              <Field label="What do you need help with?"><select name="service" className={inputClassName} defaultValue={services[0]} disabled={loading}>{services.map((service) => <option key={service}>{service}</option>)}</select></Field>
              <Field label="Message or requirements"><textarea required name="requirements" rows={3} placeholder="Tell us what you need help with." className="min-h-28 w-full rounded-xl border border-border bg-background px-4 py-3 text-sm text-foreground outline-none transition-all placeholder:text-muted-foreground focus:border-accent focus:ring-4 focus:ring-accent/15 disabled:cursor-not-allowed disabled:opacity-60" disabled={loading} /></Field>
...
type SocialAuditForm = { name: string; email: string; phone: string; brand: string; profile: string; service: string; requirements: string };
...
function validateForm(values: SocialAuditForm): string | null {
  ...
  if (!values.brand) return "Please enter your company or business name.";
