...
export default function PreviewPage({
  ...
  const isQuestionVisible = useCallback(
    (question: Question, selected: Record<number, Option | null>): boolean => {
      ...
      if (
        ...
        !Array.isArray(question.dependentOn) ||
        question.dependentOn.length === 0
      ) {
        return true;
      }
      ...
      for (const qIdxStr of Object.keys(groups)) {
        ...
        const selectedOptIdx = questions[qIdx].options.findIndex(
          (o) => o.title === answer.title,
        );
  ...
  useEffect(() => {
    ...
    const visible = questions.filter((q) =>
      isQuestionVisible(q, selectedOptions),
    );
  ...
  const isCustomFieldVisible = (field: CustomField) => {
    ...
    const selectedIndex = questions[
      field.visibility.questionIndex
    ].options.findIndex((opt) => opt.title === selected.title);
  ...
  const updateCostItems = useCallback(() => {
    ...
    const newCostItems = Object.entries(selectedOptions)
      .map(([index, option]) => {
  ...
  }, [visibleQuestions.length, costItems.length, totalEstimate, department]);
  ...
  const handleOptionSelect = (opt: Option) => {
    ...
    const visibleIndexes = newVisible.map((q) =>
      questions.findIndex((qq) => qq.questionText === q.questionText),
    );
  ...
  const hasMultiLineSubtitle =
    currentQuestion?.options.some(
      (opt) => opt.subtitle && opt.subtitle.includes("|"),
    ) || false;
  ...
  const validate = () => {
    ...
    visibleCustomFields.forEach((field) => {
      ...
      if (field.required && !value) {
        nextCustomErrors[field.id] =
          `${field.question || field.label || "This field"} is required.`;
        isValid = false;
        return;
      }
  ...
  const currentQuestionNumber = workflowSteps
...
<Firstsection/> */}
      ...
      <div className="w-full h-full relative bg-no-repeat bg-center bg-cover  lg:pt-5 pt-0 md:py-0 container ">
        <div
        ...
        >
          <section className="w-full flex flex-col lg:mt-8 pb-10 sm:pb-15 lg:pb-20">
                    ...
                    <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-2">
                      <div>
                        <div className="flex items-center gap-2">
                          ...
                          {currentQuestion.questionIcon?.startsWith(
                            "data:image",
                          ) ? (
                            <img
                              src={currentQuestion.questionIcon}
                              alt="icon"
                              className="w-6 h-6"
                            />
                          ) : (
                            ...
                            onClick={() => {
                              if (
                                ...
                                const origIdx = questions.findIndex(
                                  (q) =>
                                    q.questionText ===
                                    visibleQuestions[currentVisibleIdx]
                                      .questionText,
                                );
  ...
  `}
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-4 lg:gap-10 gap-5">
                      {currentQuestion.options.map((opt, i) => {
                        const active =
                          selectedOptions[originalIndex]?.title === opt.title;
                        return (
                          <button
                            key={i}
                            type="button"
                            ...
                            {opt.subtitle && opt.subtitle.trim() !== "" ? (
                              <ul className="flex flex-wrap md:text-[12px] lg:text-[14px] text-[15px] leading-tight font-poppins text-[#444] list-disc ml-5 ">
                                {opt.subtitle.split("|").map((item, i) => (
                                  <li
                                    key={i}
                                    className="basis-1/2 flex-shrink-0 break-words"
                                  >
                                    {item.trim()}
                                  </li>
                                ))}
...
shadow-[0_20px_60px_rgba(0,0,0,0.06)]
  ...
  "
                            ...
                            {costItems.map((item, index) => (
                              <div
                                key={index}
                                className="flex justify-between text-[15px] mb-3"
                              >
                                <p>
                            ...
                            <p className="text-[#1E1E1E] text-[13px] text-center font-[500] py-1 ">
                              This is your starting Investment Guide final
                  ...
                  <div className="relative w-full perspective min-h-[400px]">
                    <div>
                      ...
                      <div id="contact-form" className="  ">
                        <div
                          className="w-full h-auto min-h-full rounded-[20px]
...
shadow-[0_20px_60px_rgba(0,0,0,0.06)] lg:p-8 p-4 text-white relative overflow-hidden"
  ...
  "
      ...
      "
                              >
                                <Phone size={18} className="text-[#F9B31B]" />

                                <span>+91 9833037816</span>
                              </a>
      ...
      "
                              >
                                <Mail size={18} className="text-[#F9B31B]" />
